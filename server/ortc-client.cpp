...
namespace mediasoupclient
{
	namespace ortc
	{
		...
		void validateRtpCapabilities(json& caps)
		{
			...
			{
				SRV_THROW_TYPE_ERROR("caps.codecs is not an array");
			}
			...
			{
				SRV_THROW_TYPE_ERROR("caps.headerExtensions is not an array");
			}
		...
		void validateRtpHeaderExtension(json& ext)
		{
			...
			if (kindIt == ext.end() || !kindIt->is_string())
				SRV_THROW_TYPE_ERROR("missing ext.kind");
		...
		void validateIceCandidate(json& params)
		{
			...
			if (protocolMatch.empty())
				SRV_THROW_TYPE_ERROR("invalid params.protocol");
			...
			{
				SRV_THROW_TYPE_ERROR("missing params.type");
			}
			...
			if (typeMatch.empty())
				SRV_THROW_TYPE_ERROR("invalid params.type");
		}
		...
		void validateIceCandidates(json& params)
		{
			...
			if (!params.is_array())
				SRV_THROW_TYPE_ERROR("params is not an array");
		...
		void validateDtlsFingerprint(json& params)
		{
			...
			{
				SRV_THROW_TYPE_ERROR("missing params.algorithm");
			}

			// foundation is mandatory.
			if (valueIt == params.end() || (!valueIt->is_string() || valueIt->get<std::string>().empty()))
			{
				SRV_THROW_TYPE_ERROR("missing params.value");
			}
		}
		...
		void validateDtlsParameters(json& params)
		{
			...
			{
				SRV_THROW_TYPE_ERROR("missing params.role");
			}
		...
		nlohmann::json reduceCodecs(nlohmann::json& codecs, const nlohmann::json* capCodec)
		{
			...
			else
			{
				...
				if (filteredCodecs.size() == 0)
					SRV_THROW_TYPE_ERROR("no matching codec found");
			}

...
void ProducerVideoQualityController::addOrUpdateConsumer(const std::string& consumerId, bool paused, int layer) 
{
...
void ProducerVideoQualityController::removeConsumer(const std::string& consumerId) 
{
    if (m_mapConsumedStatusInfo.contains(consumerId)) {
...
bool ProducerVideoQualityController::isAllConsumerPaused()
{
    bool ret = true;
...
int ProducerVideoQualityController::getMaxDesiredQ() 
{
    auto ret = -1;
